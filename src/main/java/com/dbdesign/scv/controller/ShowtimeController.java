...
public class ShowtimeController {
    ...
    // 상영일정 등록 (어드민)
    @PostMapping
    @ApiOperation(value = "상영일정 등록", notes = "영화 관리자는 영화번호, 상영관번호, 상영일, 상영회차, 상영시작시간을 입력해 상영 일정을 등록합니다.")
    ...
    // 상영일정 수정 (어드민)
    @PatchMapping()
    @ApiOperation(value = "상영일정 수정", notes = "영화 관리자는 상영 일정을 수정할 수 있으며, 상영되지 않은 회차이며 예매된 티켓이 없는 경우 수정이 가능합니다. \n" + "" +
            "")
    ...
    @ApiOperation(value = "상영일정 삭제", notes = "물리적 삭제를 하며, 상영되지 않은 회차이며 예매된 티켓이 없는 경우 삭제가 가능합니다.")
    ...
    // 상영일정 리스트 반환 (어드민)
    @GetMapping("/list")
    @ApiOperation(value = "모든 상영일정 리스트 반환", notes = "모든 상영일정 리스트를 반환합니다.")
